...
class Items extends Model
{
	...
	{
		$select	= "SELECT items.*, users.FNAME AS 'seller_fname', users.LNAME AS 'seller_lname', countries.EXCHANGE_RATE AS 'exchange_rate' FROM items, users, countries WHERE countries.ID = users.COUNTRY_ID AND items.SELLER_ID = users.ID AND items.ID IN (SELECT order_details.ITEM_ID  FROM order_details, orders WHERE order_details.ORDER_ID = orders.ID AND orders.STATUS = 'PURCHASED' AND orders.USER_ID = $userID)";
        $stmt = self::$_connection->prepare($select);
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_CLASS, $this->_className);
        $returnVal = [];
